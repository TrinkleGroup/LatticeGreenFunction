...
while(my $line = <DISLXYZFILE>) {
	...
	my $regionnum;
	if($regiontype =~ /^$elems[0]_(\d+)/) {
		$regionnum = 0;
		$atomtype = $1;
	} elsif($regiontype =~ /^$elems[1]_(\d+)/) {
		$regionnum = 1;
		$atomtype = $1;
	} elsif($regiontype =~ /^$elems[2]_(\d+)/) {
		$regionnum = 2;
		$atomtype = $1;
	} else {
		die "Error: Invalid atom in xyz file: $dislxyzfilename!\n";
	}
	push(@$atompos, $atomtype);
...
foreach my $atompos2 (@{$d_atoms[1]}) {
	for(my $regionnum = 0; $regionnum < 3; $regionnum++) {
		foreach my $atompos (@{$d_atoms[$regionnum]}) {
			my $atomtype1 = $$atompos[4];
			my $atomtype2 = $$atompos2[4];

			my $adiff = vecdiff($atompos,$atompos2);
			my $rdiff = vecdiff($adiff, $Rdiff[$atomtype1-1]->[$atomtype2-1]);
			my $nearestlvec = searchnearest(\%cell, $rdiff);
			my $mag = vecmag2(vecdiff($rdiff,$nearestlvec));

			push(@newrgrid, $nearestlvec);
			printf LGFPFILE "%d %d %+.12f %+.12f %+.12f %d %d\n", $$atompos2[3], $$atompos[3], $$nearestlvec[0], $$nearestlvec[1], $$nearestlvec[2], $atomtype2, $atomtype1;
			printf STDERR "%.4f %d %d %d %d\n", sqrt($mag), $$atompos2[3], $$atompos[3], $atomtype2, $atomtype1;
		}
	}
}
close(LGFPFILE);
